...
Modulation of the injection current is the fastest feedback method available to diode lasers and is able to suppress noise to at least \unit[1]{MHz}.
...
\caption{Littrow configuration for diode lasers. The raw output of the diode is collimated and then incident on an optical grating. The angle of the grating changes what frequency of light is coupled back into the diode and thus the frequency of output.}
...
Thus the grating angle can be used to deal with relatively low frequency noise, up to \unit[1]{kHz} or so, such as changes to ambient temperature or pressure.
...
Some sources of noise are thermal changes, ambient vibrations, atmospheric pressure, or changes in the electrical power suppply.
