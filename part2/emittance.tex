...
For a \gls{caes} the emittance at the source is given by~\cite{mcculloch_high_coherence_2013}
...
The pepperpots were cut from \unit[25]{$\mu m$} thick copper pinhole sample with \unit[50]{$\mu m$} diameter holes cut in the centre {\color{red}(make and model number)}.
...
{\color{red}Figure showing flattop and gaussian profiles, and extrapolations of beam size.}

power?

\subsubsection{Ionisation Laser}
The ionisation laser was a {\color{red}wavelength range} \unit[x-y][nm], \unit[10][nm] pulse laser which ionisates the atoms excited by the red excitation laser.
