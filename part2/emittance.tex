...
A number of two-dimensional pepperpot measurements were taken in order to verify that the pepperpot mask parameters were appropriate and to test the analysis procedure available against the expected theoretical results. 
...
Maybe move this sections to an earlier chapter to discuss registration in one location.}
...
In order to reduce the value of the error functions below 0.05 then 261 electrons are required for the mean and 129 for the standard deviation.

\begin{figure}
    \center
    \input{part2/Figs/gaussian_sim.pgf}
    \caption{Results from a simulation of the accuracy of sample mean and standard deviation given the size of the sample. For each value of sample size the simulation are repeated 10,000 times. The red dashed line indicates an error value of 0.05 and it intersects the mean (blue) at X and the standard deviation (STD, green) at Y.}
    \label{figure:gaussian_simulation}
\end{figure}

In order to extrapolate this estimate to the number of electron required for single-shot streaked emittance measurements....
