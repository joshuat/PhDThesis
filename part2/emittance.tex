...
A number of two-dimensional pepperpot measurements were taken in order to verify that the pepperpot mask parameters were appropriate and to test the analysis procedure against the expected theoretical emittance shown in Equation~\ref{equation:excess_energy_emittance}.
...
\begin{figure}
    ...
    \caption{Single-Shot is a single image from a streaked pepperpot dataset of 1000 images. Average is the simple of all 1000 images. Slipped is an attempt at registration that only looks for the maximum value of the convolution with no limit on shot-to-shot variation, there is an additional, faint beamlet visible at the top of the image. Registration is a sucessful registration where a shot-to-shot limit to drift of 5 pixels was enforced. Below each image is a trace of the row sums of image. All the images are independently log scaled.}
